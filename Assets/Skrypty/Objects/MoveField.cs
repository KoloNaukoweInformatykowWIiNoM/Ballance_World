...
public class MoveField : MonoBehaviour {
	...
	private void PrepareDirection( int index ) {
		...
		if ( directions[ index ] == "F" ) { nextz = Round(actualz) + move[ index ]; }
		if ( directions[ index ] == "B" ) { nextz = Round(actualz) - move[ index ]; }
		if ( directions[ index ] == "L" ) { nextx = Round(actualx) - move[ index ]; }
		if ( directions[ index ] == "R" ) { nextx = Round(actualx) + move[ index ]; }
		if ( directions[ index ] == "W" ) { WaitStart( move[ index ] ); }
	}
	...
	private bool Move( string direction, int speed ) {
		...
		if (direction == "F") {
			if ( ((float)nextz) >= actualz ) { transform.Translate(Vector3.forward * (Time.deltaTime * speed)); result = false; }
			else { result = true; }
		}
		else if (direction == "B") {
			if ( ((float)nextz) < actualz ) { transform.Translate(Vector3.back * (Time.deltaTime * speed)); result = false; }
			else { result = true; }
		}
		else if (direction == "L") {
			if ( ((float)nextx) < actualx ) { transform.Translate(Vector3.left * (Time.deltaTime * speed)); result = false; }
			else { result = true; }
		}
		else if (direction == "R") {
			if ( ((float)nextx) >= actualx ) { transform.Translate(Vector3.right * (Time.deltaTime * speed)); result = false; }
			else { result = true; }
		}
		else { result = true; }
